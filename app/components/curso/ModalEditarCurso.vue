...
import type { Curso, ColorCurso } from '~/types/curso'   // ← importar ColorCurso
...
// ── Colores disponibles ───────────────────────────────────────────────────────
// ✅ FIX: id tipado como ColorCurso → el array es ColorCurso[], find() nunca falla
...
watch(() => props.modelValue, (open) => {
  ...
  errores.nombre = ''
  if (props.curso) {
    form.nombre      = props.curso.nombre      ?? ''
    form.descripcion = props.curso.descripcion ?? ''
    form.color       = props.curso.color       ?? 'emerald'
  } else {
    form.nombre      = ''
    form.descripcion = ''
    form.color       = 'emerald'
...
async function handleSubmit() {
  ...
  const cursoData: Curso = {
    id:          props.curso?.id      ?? crypto.randomUUID(),
    nombre:      form.nombre.trim(),
    descripcion: form.descripcion.trim() || null,
    color:       form.color,
    sesiones:    props.curso?.sesiones ?? [],
    creadoEn:    props.curso?.creadoEn ?? new Date().toISOString(),
  }
